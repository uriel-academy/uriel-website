...
class _StudentHomePageState extends State<StudentHomePage>
      ...
      builder: (context, snapshot) {
        ...
        return Column(
          children: [
            ...
            const SizedBox(height: 16),
            _buildOverallGradeSummary(distribution, studentIds.length),
          ],
        );
      },
    );
  ...
  Widget _buildGradeChip(int grade, int count, Color color, {int? totalStudents}) {
  ...
  // Normalization helper removed; dashboard now queries materialized aggregates
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              if (isSmallScreen) {
                ...
                return Column(
                  children: [
                    _buildSubjectPerformancePieChart(),
                    const SizedBox(height: 16),
                    _buildWeeklyProgressBarChart(),
                    const SizedBox(height: 16),
                    _buildTriviaPerformanceChart(),
                  ],
                );
              } else {
                // Desktop: Two charts per row
                return Column(
                  children: [
                    Row(
                      children: [
                        Expanded(child: _buildSubjectPerformancePieChart()),
                        const SizedBox(width: 16),
                        Expanded(child: _buildWeeklyProgressBarChart()),
                      ],
                    ),
                    const SizedBox(height: 16),
                    _buildTriviaPerformanceChart(),
  ...
  Widget _buildSubjectPerformancePieChart() {
    // Simple custom pie chart using basic Flutter widgets
    return Container(
      height: 200,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        ...
        children: [
          ...
          if (_subjectTimeSpent.isEmpty) ...[
            ...
            Expanded(
              child: Row(
                children: [
                  // Simple bar representation showing study time
                  Expanded(
                    flex: 3,
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildWeeklyProgressBarChart() {
    // Generate sample weekly data (in real app, this would come from actual data)
    final weeklyData = [
      {'day': 'Mon', 'questions': 5, 'score': 75.0},
      {'day': 'Tue', 'questions': 8, 'score': 82.0},
      {'day': 'Wed', 'questions': 3, 'score': 90.0},
      {'day': 'Thu', 'questions': 12, 'score': 78.0},
      {'day': 'Fri', 'questions': 6, 'score': 85.0},
      {'day': 'Sat', 'questions': 15, 'score': 88.0},
      {'day': 'Sun', 'questions': 4, 'score': 92.0},
    ];
  ...
  Widget _buildTriviaPerformanceChart() {
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                'Complete trivia challenges to see analytics',
                style: GoogleFonts.montserrat(
                  fontSize: 12,
                  color: Colors.grey[500],
                ),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          '${((correct / triviaQuestionsAnswered) * 100).toInt()}%',
                          style: GoogleFonts.montserrat(
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
                            color: Colors.green,
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          '${((incorrect / triviaQuestionsAnswered) * 100).toInt()}%',
                          style: GoogleFonts.montserrat(
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
                            color: Colors.red,
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text(
                    '${triviaQuestionsAnswered - triviaCorrect} Incorrect',
                    style: GoogleFonts.montserrat(
                      fontSize: 10,
                      color: Colors.grey[700],
                    ),
  ...
  void _showMessageDetailDialog(Map<String, dynamic> notification) {
    ...
    if (isMobile) {
      ...
      // Desktop: Positioned dialog (existing code)
      showDialog(
        context: context,
        barrierColor: Colors.transparent,
        builder: (context) {
...
class _TeacherDashboardWidgetState extends State<TeacherDashboardWidget> {
  ...
  void _setupRealtimeUpdates() {
    ...
    // Listen for changes to student summaries for this teacher
  ...
  Future<void> _loadData() async {
    ...
    try {
      final studentSummariesSnap = await FirebaseFirestore.instance
          .collection('studentSummaries')
          .where('teacherId', isEqualTo: _teacherId)
          .get();
      ...
      debugPrint('❌ Teacher Dashboard Error loading data: $e');
    }
  }
  ...
  Future<void> _loadSubjectMasteryData() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _subjectMastery = mastery;
          _timeBySubject = timeSpent;
        });
      }
    } catch (e) {
  ...
  void _processStudentData(List<QueryDocumentSnapshot> docs) {
    final students = docs.map((doc) {
      ...
      return {
        ...
        'questionsSolved': data['totalQuestions'] ?? 0,
        'avgPercent': data['avgPercent'] ?? 0,
        'subjectsSolved': data['subjectsCount'] ?? 0,
        'rank': data['rankName'] ?? 'Learner',
      };
    }).toList();

    final totalCount = students.length;
    debugPrint('🔍 Teacher Dashboard: Processed ${students.length} students');
  ...
  void _showStudentDialog(Map<String, dynamic> studentData) {
    showDialog(
      context: context,
  ...
  Widget build(BuildContext context) {
    ...
    for (final student in students) {
      ...
      totalQuestions += (data['questionsSolved'] as int?) ?? 0;

      final accuracy = (data['avgPercent'] as num?)?.toDouble() ?? 0;
      if (accuracy > 0) {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              '$_school • ${totalStudents == 1 ? '1 student' : '$totalStudents students'}',
              style: GoogleFonts.montserrat(
                fontSize: isSmallScreen ? 14 : 16,
                color: Colors.grey[600],
              ),
            ...
            const SizedBox(height: 16),
            _buildTeacherGradePredictionAnalytics(isSmallScreen),
            const SizedBox(height: 32),
            ...
            if (totalStudents > 0) ...[
              ...
              const SizedBox(height: 16),
              _buildStudentsAtRisk(students, isSmallScreen),
              const SizedBox(height: 32),
            ],
            ...
            if (totalStudents > 0) ...[
              ...
              const SizedBox(height: 16),
              _buildPerformanceDistribution(students, isSmallScreen),
              const SizedBox(height: 32),
            ],
            ...
            if (totalStudents > 0) ...[
              ...
              const SizedBox(height: 16),
              _buildXPDistribution(students, isSmallScreen),
              const SizedBox(height: 32),
            ],
  ...
  Widget _buildTeacherGradePredictionAnalytics(bool isSmallScreen) {
    if (_teacherId == null) return const SizedBox.shrink();

    return FutureBuilder<List<String>>(
      future: _getTeacherStudentIds(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Container(
            padding: const EdgeInsets.all(24),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 6),
                          Text(
                            'This shows how many students in your class are predicted to achieve each BECE grade (1-9) based on their quiz performance. Our AI model requires each student to complete at least 20 quizzes with 40% topic diversity to make reliable predictions.',
                            style: GoogleFonts.montserrat(
                              fontSize: 12,
                              color: Colors.grey[700],
                              height: 1.4,
  ...
  Future<List<String>> _getTeacherStudentIds() async {
    try {
      // Use the existing teacher data from widget
      if (_grade == null || _school == null) return [];

      // Find all students in the same class
      final studentsQuery = await FirebaseFirestore.instance
          .collection('users')
          .where('role', isEqualTo: 'student')
          .where('class', isEqualTo: _grade)
  ...
  Widget _buildPerformanceAnalytics(bool isSmallScreen) {
    ...
    // Get subjects with data and sort by average score
    final subjectsWithData = _subjectMastery.entries
        .map((entry) => {
              'subject': entry.key,
              'average': entry.value['average'] as double,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          ...subjectsWithData.map((data) {
            final subject = data['subject'] as String;
            final avgScore = data['average'] as double;
            final count = data['count'] as int;
  ...
  Widget _buildStudentsAtRisk(List students, bool isSmallScreen) {
    final atRiskStudents = students.where((s) {
      final student = s as Map<String, dynamic>;
      final accuracy = (student['avgPercent'] as num?) ?? 0;
        ...
        children: [
          ...
          if (atRiskStudents.isEmpty) ...[
            ...
            ...atRiskStudents.map((s) {
              ...
              final accuracy = (student['avgPercent'] as num).toDouble();
              final questions = student['questionsSolved'] as int;
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Text(
                      '$count student${count != 1 ? 's' : ''} (${percentage.toStringAsFixed(0)}%)',
                      style: GoogleFonts.montserrat(
                        fontSize: 12,
                        color: Colors.grey[600],
                      ),
        ...
        children: [
          ...
          if (topStudents.isEmpty) ...[
            ...
            ...topStudents.asMap().entries.map((entry) {
              ...
              final name = student['displayName'] ?? 'Unknown';
              final xp = student['totalXP'] as int? ?? 0;
  ...
  Widget _buildRankDistribution(List students, bool isSmallScreen) {
    final rankCounts = <String, int>{};
    for (var s in students) {
      final student = s as Map<String, dynamic>;
      final rank = student['rank'] as String? ?? 'Learner';
    ...
    final rankColors = {
      ...
      'Champion': const Color(0xFFC0C0C0),
      'Master': const Color(0xFFCD7F32),
      'Expert': const Color(0xFF9C27B0),
      'Scholar': const Color(0xFF2196F3),
      'Achiever': const Color(0xFF4CAF50),
      'Learner': const Color(0xFF757575),
    };
...
class _StudentDetailDialogState extends State<StudentDetailDialog> {
  ...
  void _setupRealtimeUpdates() {
    // Listen for changes to the student's quizzes
    _quizzesSubscription = FirebaseFirestore.instance
        .collection('quizzes')
        .where('userId', isEqualTo: widget.studentData['uid'])
  ...
  Future<void> _loadDetailedData() async {
    try {
      // Load detailed student information including recent quizzes
      ...
      _processQuizData(quizzesSnap.docs);
    } catch (e) {
      debugPrint('Error loading detailed student data: $e');
      setState(() {
        _isLoading = false;
      });
    }
  ...
  Map<String, dynamic> _calculateStudentStats(
    ...
    int beceQuestions = 0;
    int beceCorrect = 0;
    int triviaQuestions = 0;
    int triviaCorrect = 0;

    for (final quiz in quizzes) {
      final questions = (quiz['totalQuestions'] as int?) ?? 0;
      final correct = (quiz['correctAnswers'] as int?) ?? 0;
      final subject = quiz['subject'] ?? 'Unknown';
      final percentage = (quiz['percentage'] as num?)?.toDouble() ?? 0.0;
    ...
    subjectScores.forEach((subject, scores) {
      ...
      subjectMastery[subject] = {
        'average': avgScore,
        'totalQuestions': subjectQuestionCounts[subject] ?? 0,
        'totalCorrect': subjectCorrectCounts[subject] ?? 0,
        'quizCount': scores.length,
      };
    });

    // Study time by subject (assume 2 minutes per question)
    Map<String, double> studyTimeBySubject = {};
    subjectQuestionCounts.forEach((subject, questions) {
      studyTimeBySubject[subject] = questions * 2.0; // minutes
    });
    ...
    return {
      ...
      'becePerformance': {
        'average': beceAverage,
        'totalQuestions': beceQuestions,
        'totalCorrect': beceCorrect,
      },
      'triviaPerformance': {
        'average': triviaAverage,
        'totalQuestions': triviaQuestions,
        'totalCorrect': triviaCorrect,
      },
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${widget.studentData['totalXP'] ?? 0} XP',
                          style: GoogleFonts.montserrat(
                            fontSize: 14,
                            color: Colors.white.withValues(alpha: 0.8),
                          ),
