...
import '../widgets/enhanced_message_dialog.dart';
...
class _SchoolAdminStudentsPageState extends State<SchoolAdminStudentsPage> {
  ...
  Future<void> _loadSchoolContext() async {
    ...
    try {
      ...
      final data = doc.data();
      setState(() {
        _schoolName = data?['school'] as String?;
      });
  ...
  Future<void> _showSendMessageDialog() async {
    ...
    // Fetch ALL students for messaging (not just current page)
    List<Map<String, dynamic>> allSchoolStudents = [];
    try {
      final functions = FirebaseFunctions.instanceFor(region: 'us-central1');
      final callable = functions.httpsCallable('getSchoolStudents');
      final result = await callable.call({'schoolId': _schoolName});
      final data = result.data as Map<String, dynamic>;
      allSchoolStudents = (data['students'] as List).cast<Map<String, dynamic>>();
    } catch (e) {
      // Fallback to cached students if function fails
      allSchoolStudents = allStudents;
    }
    ...
    await showDialog(
      ...
      builder: (context) => EnhancedMessageDialog(
        allUsers: allSchoolStudents,
