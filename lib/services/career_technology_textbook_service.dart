...
/// Service for managing CareerTechnology textbooks with XP system
/// Loads from local JSON assets for instant performance
class CareerTechnologyTextbookService {
  static final CareerTechnologyTextbookService _instance = CareerTechnologyTextbookService._internal();
  factory CareerTechnologyTextbookService() => _instance;
  CareerTechnologyTextbookService._internal();
  ...
  /// Load all CareerTechnology textbooks from JSON assets into cache
  Future<void> _loadTextbooksFromAssets() async {
    ...
    try {
      ...
      for (final id in textbookIds) {
        try {
          print('📚 [CareerTechnology] Loading: assets/textbooks/$id.json');
      ...
      _cacheLoaded = true;
      print('📖 Loaded ${_textbookCache.length} CareerTechnology textbooks from assets');
      print('   Cached IDs: ${_textbookCache.keys.join(', ')}');
    } catch (e) {
      print('❌ Error loading CareerTechnology textbooks from assets: $e');
    }
  }

  /// Get all CareerTechnology textbooks (from JSON assets with Firestore fallback)
    ...
    try {
      ...
      if (_textbookCache.isNotEmpty) {
        final textbooks = _textbookCache.values.map((textbook) {
          return {
            'id': textbook['id'] ?? '',
            'subject': textbook['subject'] ?? 'CareerTechnology',
            'year': textbook['year'] ?? '',
      ...
      print('Error getting CareerTechnology textbooks: $e');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>?> getTextbook(String year) async {
    try {
      ...
      print('Error getting CareerTechnology textbook: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>> getUserProgress(String textbookId) async {
    try {
      ...
      final doc = await _firestore
          .collection('users')
          .doc(userId)
          .collection('CareerTechnology_textbook_progress')
          .doc(textbookId)
          .get();
