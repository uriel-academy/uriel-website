...
/// Service for managing Science textbooks with XP system
/// Loads from local JSON assets for instant performance
class ScienceTextbookService {
  static final ScienceTextbookService _instance = ScienceTextbookService._internal();
  factory ScienceTextbookService() => _instance;
  ScienceTextbookService._internal();
  ...
  /// Load all science textbooks from JSON assets into cache
  Future<void> _loadTextbooksFromAssets() async {
    if (_cacheLoaded) return;
    
    try {
      final textbookIds = ['science_jhs_1', 'science_jhs_2', 'science_jhs_3'];
      ...
      for (final id in textbookIds) {
        try {
          print('📚 [Science] Loading: assets/textbooks/$id.json');
      ...
      _cacheLoaded = true;
      print('📖 Loaded ${_textbookCache.length} science textbooks from assets');
      print('   Cached IDs: ${_textbookCache.keys.join(', ')}');
    } catch (e) {
      print('❌ Error loading science textbooks from assets: $e');
    }
  }

  /// Get all Science textbooks (from JSON assets with Firestore fallback)
  Future<List<Map<String, dynamic>>> getAllTextbooks() async {
    try {
      ...
      print('Error getting science textbooks: $e');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>?> getTextbook(String year) async {
    try {
      ...
      print('Error getting science textbook: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>> getUserProgress(String textbookId) async {
    try {
      ...
      final doc = await _firestore
          .collection('users')
          .doc(userId)
          .collection('science_textbook_progress')
          .doc(textbookId)
          .get();
