...
(async () => {
  const snapshot = await db.collection('questions').where('subject', '==', 'religiousMoralEducation').limit(20).get();
  console.log('Found', snapshot.size, 'docs with subject religiousMoralEducation');
