...
// Asante Twi Topics (12 topics based on curriculum strands)
...
async function createAsanteTwiTopicCollections() {
  try {
    console.log('🇹🇼 Creating Asante Twi Topic Collections...\n');

    const indexPath = path.join('assets', 'asante_twi_questions_by_topic', 'index.json');
    ...
    for (const topic of ASANTE_TWI_TOPICS) {
      ...
      const topicFilePath = path.join('assets', 'asante_twi_questions_by_topic', topicData.filename);
      ...
      const collectionId = `asante_twi_topic_${topicKey}`;
      const collectionRef = db.collection('questionCollections').doc(collectionId);

      const collectionData = {
        id: collectionId,
        name: `BECE Asante Twi ${topic}`,
        description: `Practice ${topicData.questionCount} Asante Twi questions on ${topic}`,
        subject: 'asanteTwi',
        examType: 'bece',
        type: 'topic',
        topic: topic,
    ...
    console.log(`\n✅ Successfully created ${totalCollectionsCreated} Asante Twi topic collections!`);
    ...
    console.error('❌ Error creating Asante Twi topic collections:', error);
    throw error;
  }
}
...
// Run the function
createAsanteTwiTopicCollections()
  .then(() => {
    console.log('\n🎉 Asante Twi topic collections creation completed successfully!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n💥 Failed to create Asante Twi topic collections:', error);
    process.exit(1);
  });