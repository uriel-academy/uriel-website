...
async function checkCreativeArtsCollections() {
  try {
    const collections = await db.collection('questionCollections')
      .where('subject', '==', 'creativeArts')
      .get();

    console.log('Creative Arts collections in questionCollections:');
...
checkCreativeArtsCollections().then(() => process.exit(0));