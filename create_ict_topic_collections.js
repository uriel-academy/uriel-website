...
async function createICTTopicCollections() {
  try {
    console.log('📚 Creating ICT Topic Collections...\n');
    
    const indexPath = path.join('assets', 'ict_questions_by_topic', 'index.json');
    ...
    for (const topicInfo of indexData.topics) {
      ...
      const topicFilePath = path.join('assets', 'ict_questions_by_topic', filename);
      ...
      const collectionId = `ict_topic_${filename.replace('.json', '')}`;
      const collectionRef = db.collection('questionCollections').doc(collectionId);

      const collectionData = {
        id: collectionId,
        name: `ICT: ${topicName}`,
        description: `Practice ${questionCount} ICT questions on ${topicName}`,
        subject: 'ict',
        examType: 'bece',
      ...
      console.log(`✅ Created: ICT: ${topicName} (${questionCount} questions)`);
    ...
    console.log(`Total ICT Topic Collections Created: ${totalCollectionsCreated}`);
    console.log('='.repeat(70));
    
    console.log('\n✅ All ICT topic collections have been created in Firestore!');
    console.log('📍 Collection: questionCollections');
    console.log('\n🎯 Next Steps:');
    console.log('   1. The collections are now available in the Questions page');
    console.log('   2. Users can filter by Subject: ICT to see all topic collections');
